...
class SplineRoad:
    def __init__(self, s=0.25, a=40000, b=40000, start=0, end=1, count_cone=100):
        self.s = s
        self.a = a
        self.b = b
        self._maxfunc = None
    ...
    def generate_data(self, output=True):
        ...
        np.random.seed(1000)
        data_x = np.concatenate([[0], np.sort(np.random.rand(6)), [1]])
        data_y = 0+ np.sin(np.concatenate([[0], np.random.rand(6), [1]]))
    ...
    def plot_cones(self, plot_func: bool = True, accuracy: int = 200):
        ...
        plt.plot(self.transform_x(self.lcx-self.s), self.transform_y(self.lcy-self.s/2), 'o', c='Red')
        plt.plot(self.transform_x(self.xxc), self.transform_y(self.ycc), 'o', c='Red')
        plt.plot(self.transform_x(self.rcx+self.s), self.transform_y(self.rcy+self.s/2), 'o', c='Red')
        plt.show()
